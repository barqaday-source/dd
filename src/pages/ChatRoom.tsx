...
import { ArrowRight, Send, Loader2, Settings2, Users, Trash2, EyeOff, X, Image as ImageIcon, Pencil, Check } from "lucide-react";
...
export default function ChatRoom() {
  ...
  const handleSend = async (e: FormEvent) => {
    ...
    setSending(true);
    const { error } = await sendMessage({ content: input, type: "text" }, user.id);
    setSending(false);
    if (error) toast.error("فشل الإرسال", { description: error.message });
    else { setInput(""); setTyping(false); }
  };
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col max-w-[500px] mx-auto">
      <header className="h-[65px] px-4 flex items-center gap-3 bg-card border-b border-border/50 safe-top sticky top-0 z-40">
              ...
              <div key={msg.id} className={`flex gap-2 anim-fade-in ${mine ? "flex-row-reverse" : ""}`}>
                ...
                <div className={`max-w-[75%] ${mine ? "items-end" : "items-start"} flex flex-col`}>
                  ...
                  <button
                    ...
                    className={`px-3 py-2 rounded-2xl text-sm break-words text-right ${
                      authorIsAdmin && !mine
                  ...
                  >
                    {msg.message_type === "image" && msg.media_url ? (
                      <img
                        ...
                        className="max-w-[240px] max-h-[300px] rounded-xl cursor-zoom-in"
                      />
      ...
      {room?.status === "pending" ? (
        ...
        <form
          onSubmit={handleSend}
          className="px-3 pt-2 pb-3 flex gap-2 safe-bottom sticky bottom-0 items-center bg-transparent"
        >
          <input
            ref={fileInputRef}
            type="file"
